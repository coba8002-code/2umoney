...
```mermaid
flowchart TD
  A["입력: 전경색 fg · 배경색 bg · 목표 대비 target"] --> B{"현재 대비 ≥ target?"}
  B -- 예 --> Z["그대로 반환<br/>(styleImpact: none)"]
  B -- 아니오 --> C["fg를 OKLCH로 변환<br/>L 명도 · C 채도 · H 색상"]
  C --> D["채도 C · 색상 H 고정 (브랜드 톤 보존)"]
  D --> E["L을 어둡게 이분 탐색"]
  D --> F["L을 밝게 이분 탐색"]
  E --> G{"변경량이 더 작은 쪽 선택"}
  F --> G
  G --> H["결과 hex로 대비 재계산 (gamut clamp 반영)"]
  H --> I{"대비 ≥ target?"}
  I -- 예 --> Y["보정 색 반환<br/>(styleImpact: minimal)"]
  I -- 아니오 --> J["채도까지 낮춰 재시도 (fallback)"]
  J --> H
```

> 핵심: **색상·채도를 고정**한 채 명도만 양방향으로 이분 탐색하므로 브랜드 색감을 유지하고, 마지막에 **결과 색으로 다시 검증**하므로 "보정했는데 여전히 미달" 상황이 없습니다.
...
업종 시나리오를 "키오스크"로 바꾸면, 동일 엔진이 무인 단말 화면(예: 카페 메뉴)을 검사합니다. 고령자·장애인 사용이 많은 키오스크에서 특히 중요한 **터치 타깃 크기·대비**를 같은 기준으로 잡아냅니다.

![키오스크 시나리오 화면](./screens/03-kiosk.png)

### 3.4 다크 모드 (도그푸딩)

도구 자체 UI도 KWCAG/WCAG AA를 준수하도록 설계했습니다(다크 모드 포함, 가시적 포커스 표시). "접근성 도구가 정작 접근성이 나쁜" 문제를 피하기 위한 자기적용입니다.

![다크 모드 화면](./screens/04-dark.png)
